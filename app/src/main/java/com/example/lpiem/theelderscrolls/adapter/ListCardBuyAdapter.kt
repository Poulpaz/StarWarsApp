...
class ListCardBuyAdapter(private val listUserCards : List<Card>) : ListAdapter<Card, ListCardBuyAdapter.CardViewHolder>(DiffCardCallback()) {
    ...
    inner class CardViewHolder(itemView: View, private val cardsClickPublisher: PublishSubject<String>) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(card: Card) {
            if(listUserCards.contains(card)){
                itemView.group_card_bought.visibility = View.VISIBLE
            } else { itemView.group_card_bought.visibility = View.GONE }
